...
const DELIVERY_STATUS_LABEL: Record<string, string> = { pending: "Not delivered", partial: "Partial", delivered: "Delivered" };
...
export default function MonitoringTab() {
  ...
  // Delivery summary counts active (non-cancelled) POs only -- a
  // cancelled PO was never going to be delivered, so including it would
  // just dilute the picture of what's actually still outstanding.
  const deliverySummary = useMemo(() => {
    const active = purchaseOrders.filter((po) => po.status === "active");
    const counts = { pending: 0, partial: 0, delivered: 0 };
  ...
  async function setDeliveryStatus(po: PurchaseOrderRow, delivery_status: string) {
    setDeliveryBusyId(po.id);
    setErr(null);
    try {
      const res = await apiFetch(`/api/purchase-order/${po.id}/delivery`, {
      ...
      {!loading && (
        <>
          ...
          {deliverySummary.total > 0 && (
            <div className="card">
              <h2 className="text-psu font-semibold mb-1">Delivery Status Summary</h2>
              <p className="text-xs text-slate-500 mb-3">Across {deliverySummary.total} active purchase order{deliverySummary.total === 1 ? "" : "s"} (cancelled POs excluded).</p>
              <div className="space-y-2">
                {([
                  ["delivered", "Delivered", "#059669"],
                  ["partial", "Partially delivered", "#d97706"],
                  ["pending", "Not yet delivered", "#94a3b8"],
                ] as const).map(([key, label, color]) => {
                  const count = deliverySummary.counts[key];
                  const pct = deliverySummary.total > 0 ? (count / deliverySummary.total) * 100 : 0;
                  return (
                    <div key={key} className="flex items-center gap-3">
                      ...
                      <span className="text-xs text-slate-600 w-16 text-right tabular-nums">{count} ({Math.round(pct)}%)</span>
                    </div>
                  );
                })}
              </div>
              ...
              <table className="w-full text-xs">
                ...
                <tbody>
                  {poGroups.flatMap(([label, poRows]) => [
                    ...(poGroupBy !== "none" ? [
                      <tr key={`g-${label}`} className="bg-slate-50">
                        <td colSpan={canEditMonitoring ? 9 : 8} className="py-1 px-2 font-semibold text-slate-600">
                    ...
                    ...poRows.map((po) => (
                      <tr key={po.id} className="border-b border-slate-100">
                        ...
                        <td className="py-1.5 pr-2">
                          {po.status === "active" && canEditMonitoring ? (
                            <select
                              className="input text-[11px] py-0.5"
                              value={po.delivery_status}
                              disabled={deliveryBusyId === po.id}
                              onChange={(e) => setDeliveryStatus(po, e.target.value)}
                            >
                            ...
                            <span className={"px-1.5 py-0.5 rounded text-[10px] font-medium " + DELIVERY_STATUS_COLOR[po.delivery_status]}>
                              {DELIVERY_STATUS_LABEL[po.delivery_status]}
                            </span>
                          )}
                        </td>
                  ...
                  {purchaseOrders.length === 0 && (
                    <tr><td colSpan={canEditMonitoring ? 9 : 8} className="py-3 text-slate-400">No purchase orders generated yet.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
...
function GeneratePoPanel({
  ...
  // Autofill address/TIN from the Supplier Directory when this supplier's
  // name matches an entry there -- saves retyping it from scratch on every
  // PO, the way it always had to be before. Best-effort: a missing/failed
  // directory lookup just leaves the fields blank for manual entry, same
  // as before this existed.
  useEffect(() => {
    apiFetch("/api/suppliers")
      .then((r) => r.json())
      .then((j) => {
        const match = (j.rows ?? []).find((s: { name: string }) => s.name.toLowerCase() === supplier.toLowerCase());
        if (match) {
          setAddress((prev) => prev || match.address || "");
          setTin((prev) => prev || match.tin || "");
        }
      })
      .catch(() => {});
