 ...
 *  Allowed fields: course_code, course_title, description, sort_order, cost_per_title.
 *  (subjects.locked still exists in the DB but is deprecated -- locking is
 *  now per-title via assignments.manual; see /api/match/override's `lock`
 *  action. Not accepted here anymore.)
 *
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const allowed = ["course_code", "course_title", "description", "sort_order", "cost_per_title"];
    ...
    const otherFields = Object.keys(patch).some((k) => k !== "cost_per_title");
    if (otherFields) {
